...
Stage 3 is canonically implemented through **Stage 3.74 — Transaction Correction & Reversal / Ledger Repair UX**. **Stage 3.75 — Portfolio Cash Flow & Income Truth** is the current Draft PR #152 implementation candidate: engineering review remediation is complete and the pre-documentation-sync reviewed runtime head `83833ce0c791992e7f64d736dbf04e53c1de0298` passed CI #454 / run `34143875428` 10/10 SUCCESS. Stage 3.75 remains non-canonical until the final exact PR head is green after documentation synchronization, the required review/governance gates are satisfied, explicit human authorization is given, and the PR is squash-merged into protected `develop`. The current canonical product baseline includes the PostgreSQL/Go/Next.js vertical slice, authenticated portfolio and transaction flows, CSV import/reconciliation, backend-owned instrument catalog and asset discovery, hardened idempotency/security boundaries, provider-neutral market-data architecture, Corporate Actions Calendar/Heatmap API and UI, the user-supplied Dividend Calculator, deterministic manual SELL/WAC/acquisition-basis accounting, the dedicated portfolio positions API/UI, historical position reconstruction by explicit BusinessDate, and immutable transaction repair through Stage 3.74. No Stage 3.76+ product/runtime scope is authorized by the Stage 3.75 candidate.

The original Stage 3.16 repository audit is **fully remediated: 32/32 findings CLOSED (P0/P1/P2/P3 = 0/0/0/0)** through Stage 3.56. Stage 3.59 contains a delayed MOEX ISS TQBR adapter, but Stage 3.60 records production/public activation as **NO-GO** under the reviewed source-rights and zero-budget constraints, so the adapter remains dormant. Market valuation therefore remains intentionally unavailable rather than fabricated. Corporate Actions Features 3A/3B/3C are implemented provider-neutrally, while Feature 3D real-source activation remains a separate gate. Stage 3.68 Dividend Calculator and Stages 3.71–3.74 portfolio position/cost-basis, Time Machine and immutable transaction-repair flows are canonical; Stage 3.74 is protected-merged through PR #150 at `0580bf7e98c532202f84bbf9ceacd97aedbe4140` from exact final head `ff6d3efb8bb0d63f8cab55ac82fdf1052946aa02` after CI #435 / run `34117662576` completed 10/10 SUCCESS. Stage 3.25 privacy evidence collection remains documentation-only and does not authorize privacy-lifecycle implementation.
