...
| Next Review Date | Before any Stage 3.76+ product/runtime or architecture-changing scope, Stage 3.25 privacy evidence-collection plan review, Feature 3D corporate-actions source/use planning, or the next separately reviewed audit-remediation scope |

## Architecture status

**Architecture Freeze v1.2: ACTIVE**
**Documentation Freeze: ACTIVE**
**Documentation registry baseline: Stage 3.75 implementation is canonical on protected `develop` through PR #153 squash merge `57faae841805a5ab11ac959018a315cbada69207`; implementation head `f97f44016d22e783980c47bbde4799a3124623f4` passed CI #459 / run `34155729975` 10/10 SUCCESS, evidence head `fae674518e37d11cad0da28d91682c35bf8bf99a` passed CI #460 / run `34156290105` 10/10 SUCCESS, fresh External review and evidence/no-drift verification were APPROVED. This post-merge governance closure synchronizes the active registries and becomes the Stage 3.75 documentation baseline when the approved closure is present on protected `develop`.**
**Implementation frontier: Stage 3.75 — Portfolio Cash Flow & Income Truth is COMPLETE / CANONICAL through PR #153 squash merge `57faae841805a5ab11ac959018a315cbada69207`. It reuses the Stage 3.74 effective ledger for backend-owned cash-flow/income truth, preserves gross recorded dividend/coupon semantics, keeps unrelated expenses out of `netInvestmentIncome`, adds no market valuation/P&L/XIRR/provider activation, and introduces no migration, cache table, worker or paid service. Stage 3.75 is the last completed implementation stage; any Stage 3.76+ runtime or architecture-changing scope requires a separately reviewed gate.**
**Current candidate frontier: none. No Stage 3.76+ product/runtime scope is authorized by the Stage 3.75 merge or this documentation closure.**
