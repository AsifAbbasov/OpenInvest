...
| Next Review Date | Before any Stage 3.77+ product/runtime or architecture-changing scope, Stage 3.25 privacy evidence-collection plan review, Feature 3D corporate-actions source/use planning, or the next separately reviewed audit-remediation scope |

## Architecture status

**Architecture Freeze v1.2: ACTIVE**
**Documentation Freeze: ACTIVE**
**Documentation registry baseline: Stage 3.76 implementation is canonical on protected `develop` through PR #155 squash merge `e00699f8d455bcbaea0c1dc69ce534460fea6ff9`; exact final head `f89e1e793ef505c31bf76de7d43c4d40fdd9c38b` / tree `5971ae4bf5461dd562d7a73289b5942b02f962ba` passed CI #467 / run `34174642209` 10/10 SUCCESS, Internal Review was `APPROVED`, fresh External implementation re-review was `APPROVED`, evidence publication and authority synchronization completed, and no-drift verification passed. This post-merge governance closure synchronizes the active registries and becomes the Stage 3.76 documentation baseline when the approved closure is present on protected `develop`.**
**Implementation frontier: Stage 3.76 — Manual Market Valuation & Portfolio P/L is COMPLETE / CANONICAL through PR #155 squash merge `e00699f8d455bcbaea0c1dc69ce534460fea6ff9`. It adds authenticated explicit `USER_SUPPLIED` RUB manual valuation for open STOCK/BOND positions and backend-owned exact Decimal scale-8 Half-Even market value, unrealized gain/loss, unrealized return, valued-position allocation and honest partial/complete portfolio coverage. Position-generation semantics preserve correction/reversal/partial-SELL safety and prevent stale valuation reactivation after full close → reopen; historical valuation eligibility is exact-date only. It does not activate MOEX, broker, scraping, quote providers, polling, workers, Redis, Kafka, FX, XIRR/TWR, tax, notifications, AI, mobile or paid infrastructure. Stage 3.76 is the last completed implementation stage; any Stage 3.77+ runtime or architecture-changing scope requires a separately reviewed gate.**
**Current candidate frontier: none. No Stage 3.77+ product/runtime scope is authorized by the Stage 3.76 merge or this documentation closure.**
