...
| Next Review Date | Before any Stage 3.78+ product/runtime or architecture-changing scope, Stage 3.25 privacy evidence-collection plan review, Feature 3D corporate-actions source/use planning, or the next separately reviewed audit-remediation scope |

## Architecture status

**Architecture Freeze v1.2: ACTIVE**
**Documentation Freeze: ACTIVE**
**Documentation registry baseline: Stage 3.77 implementation is canonical on protected `develop` through PR #157 squash merge `63d916b447e91c4de54efee5c66b27cf7727be92`; exact final evidence head `b3bc17e710e306f9d1a4d7be95dfe56b3f7202ba` / tree `e91e28e96416d9510962a66c1399459844574bba` passed protected exact-head CI run `34303895226` 10/10 SUCCESS, pre-publication Internal Review was `APPROVED`, fresh External published-head review was `APPROVED` after remediation, evidence publication completed, and no implementation drift remained. This post-merge governance closure synchronizes the seven active registries and becomes the Stage 3.77 lifecycle/documentation baseline when the approved closure is present on protected `develop`.**
**Implementation frontier: Stage 3.77 — Portfolio Money-Weighted Return / XIRR is COMPLETE / CANONICAL through PR #157 squash merge `63d916b447e91c4de54efee5c66b27cf7727be92`. It adds backend-owned money-weighted return using Stage 3.74 correction/reversal-aware external DEPOSIT/WITHDRAWAL flows, exact-date Stage 3.76 COMPLETE `USER_SUPPLIED` RUB terminal valuation, ACT/365, Decimal scale-8 / Half-Even public output, one canonical engine for `/returns` and `PortfolioSummary.xirr`, explicit unavailable reasons, multiple-root detection and fail-closed numerical boundaries. BUY/SELL/DIVIDEND/COUPON/FEE/TAX remain internal flows, and TWR, nominal/real/inflation return activation, FX, provider/live/MOEX prices, broker sync, tax, workers, notifications, AI and mobile remain outside this stage. Stage 3.77 is the last completed implementation stage; Stage 3.78+ requires a separately reviewed gate.**
**Current candidate frontier: none. No Stage 3.78+ product/runtime scope is authorized by the Stage 3.77 merge or this documentation closure.**
