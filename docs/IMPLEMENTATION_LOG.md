...
| 1 — Documentation Consolidation | Establish the repository-owned Source of Truth and freeze v1.2 | Complete | [Stage 1 report](stages/STAGE_01_DOCUMENTATION_CONSOLIDATION.md) |
...
| 3.16 — Repository Audit Report | Record mandatory full repository audit coverage, manifest, and verdict | Complete / returned `changes required` | [Stage 3.16 audit report](stages/STAGE_03_16_REPOSITORY_AUDIT_REPORT.md) |
| 3.16 — Repository Audit Fixes | Fix mandatory repository audit `changes required` findings | Complete / closed; merged into `develop` at `9e6b8a753bf73ef020ce40461df25a5878344d92` | [Stage 3.16 audit fixes](stages/STAGE_03_16_REPOSITORY_AUDIT_FIXES.md) |
...
Canonical record: PR #59, PR #60; commit(s) `dc83f5f3a11da164e6809593861d96ccf47b29ca`, `0ddc618a3450ea81fd4befb3b10c959b3cb82a25`.
...
Canonical record: PR #164; commit(s) `247081a95a7daf33c0077c88c5f41cb2e8161865`, `ddc5b5be36f0b5ae127c6ee430918a9dba1b453e`.
...
- Kept Stage 3.7 implementation, public import endpoints, upload UI, SQL import-session
  persistence, broker/provider integrations, workers, tax logic, mobile, and  out of scope.
...
- Added no public import API, frontend upload UI, SQL import-session table, worker, provider
  integration, tax logic, mobile, or  scope.
...
- Kept public import endpoints, upload UI, SQL import-session persistence, workers,
  broker/provider integrations, tax logic, mobile, and Stage 3.8 work out of scope.
...
- Kept public import endpoints, OpenAPI changes, upload UI, SQL import-session persistence, raw
  file persistence, workers, broker/provider integrations, tax logic, mobile, and implementation
  out of scope.
...
- Added no public import API, OpenAPI changes, upload UI, SQL import-session table, raw file
  persistence, worker, provider integration, tax logic, mobile, or  scope.
...
- Kept public import endpoints, OpenAPI changes, upload UI, SQL import-session persistence, raw file
  persistence, workers, provider integrations, tax, mobile, and Stage 3.9 work out of scope.
...
- Kept OpenAPI changes, Go handlers, frontend upload UI, SQL import-session persistence, raw file
  persistence, workers, provider integrations, tax logic, mobile, and implementation out of
  scope.
...
- Kept frontend upload UI, SQL import-session persistence, workers, broker/provider integrations,
  tax, mobile, and Stage 3.10 work out of scope.
...
- Kept frontend upload UI, SQL import-session persistence, raw CSV persistence, workers,
  broker/provider integrations, tax, mobile, and Stage 3.10 implementation out of scope.
...
- Kept OpenAPI changes, backend handlers, SQL migrations, import-session persistence, raw file
  persistence, provider integrations, workers, tax, mobile, and  out of scope.

## 2026-07-09 — Stage 3.10 import upload/review UI slice closed

Canonical record: PR #27; commit(s) `e19a1a0ea4b0b183687bd89daabdfbc973daea71`.
...
- Added no frontend auth UI, email verification, OAuth/passkeys/2FA, worker, provider integration,
  tax logic, mobile implementation,  functionality, or Stage 3.12 scope.
...
Canonical record: commit(s) `8a8052c18768dbad0aa0e724836f3c9252d257e3`.
  findings.
...
- Kept frontend auth UI, email verification, OAuth/passkeys/2FA, workers, provider integrations,
  tax, mobile, and Stage 3.12 out of scope.
...
- Kept Next.js implementation, Go handler changes, OpenAPI changes, SQL migrations, token-storage
  changes, business logic, provider integrations, workers, tax, mobile, and Stage 3.13 out of
  scope.
...
- Kept Route Handlers, Server Actions, OpenAPI changes, SQL migrations, direct datastore access,
  ...
  tax, mobile, and Stage 3.13 work out of scope.
...
Canonical record: commit(s) `ca16af9adba249fc8c32c9b246b5f92f7e290b92`.
...
- Kept the fixes inside the backend/store/test/documentation implementation slice; no OpenAPI,
  migration, handler, frontend, provider, worker, market-data, tax, mobile, or  scope was added.
...
- Preserved the narrow approved fixture set and explicit unsupported ticker rejection while keeping
  ...
  mobile, tax, OpenAPI changes, SQL migrations, and Go handler changes out of scope.
...
- Kept implementation, frontend stock/bond cards, OpenAPI changes, SQL migrations, provider
  integrations, workers, market-data ingestion, financial calculations, tax, mobile, and  out of
  scope.
...
- Kept OpenAPI changes, SQL migrations, frontend stock/bond cards, providers, workers,
  market-data ingestion, financial calculations, tax, mobile, and Stage 3.15 out of scope.
...
- Kept OpenAPI changes, SQL migrations, frontend stock/bond cards, provider integrations,
  market-data ingestion, workers, financial calculations, tax, mobile, and Stage 3.15 out of
  scope.
...
Canonical record: commit(s) `f5289eb604b8ba31aa422d0d09950da02e0f48b3`.
  approval.
...
- Kept implementation, OpenAPI changes, SQL migrations, Route Handlers, Server Actions, direct
  ...
  tax, mobile, and  out of scope.

## 2026-07-26 — Stage 3.15 technical reassessment findings fixed
...
- Kept implementation work, financial algorithms, OpenAPI changes, SQL migrations, dependency
  changes, market data, providers, workers, tax, mobile, and  out of scope.
...
- Kept financial algorithms, market data, providers, workers, tax, mobile, and  out of scope.
...
- Added a durable 200-path Stage 3.16 coverage manifest and synchronized the audit report, document
  index, and version matrix. The original audit verdict remains `changes required` as historical
...
- Kept runtime code, OpenAPI, SQL migrations, backup/KMS configuration, market data, financial
  calculations, tax, mobile, and all implementation work out of scope.
...
- Kept the OpenAPI, runtime, PostgreSQL schema, key-management/provider configuration, operations,
  market data, financial calculations, tax, mobile, and all implementation work out of scope.
...
Canonical record: commit(s) `849d934906f878a6d79ba89e940e5ba470e64c09`.
...
- The earlier `changes required` remains historical evidence of the corrected
...
- Explicit merge gate was given before PR #59 merged.
...
Canonical record: PR #61, PR #62; commit(s) `7331d3f34783baec3997497d1a79b78eaa558bd4`, `f9e70e70956c76edbc2ab02c52d45124b2dea525`.
...
## 2026-09-08 — Stage 3.76 Manual Market Valuation & Portfolio P/L implementation merged

- PR #155 was squash-merged into protected `develop` at `e00699f8d455bcbaea0c1dc69ce534460fea6ff9`; protected merge tree is `5971ae4bf5461dd562d7a73289b5942b02f962ba`.
- Final evidence-authority head `f89e1e793ef505c31bf76de7d43c4d40fdd9c38b` passed exact-head GitHub Actions CI #467 / run `34174642209` with all 10 required jobs successful.
- The implementation adds authenticated `USER_SUPPLIED` manual RUB valuation for open STOCK/BOND positions, backend-owned exact Decimal scale-8/Half-Even market value and unrealized P/L, explicit partial/full coverage semantics, exact-date historical valuation, and generation-safe lifecycle behavior across correction/reversal/partial sell/full close → reopen.
- The final implementation/evidence lifecycle reached Internal `APPROVED`, fresh External `APPROVED`, evidence publication, exact-head CI and no-drift verification before explicit human Ready and squash-merge authorization.
- Stage 3.76 does not activate the dormant MOEX ISS quote adapter or any external/live provider. Stage 3.60 source-rights/public-use NO-GO remains authoritative for provider-derived shipped valuation.
- Stage 3.76 does not authorize XIRR/TWR/real-return methodology, inflation, FX, tax advice/export, broker synchronization, Redis/Kafka/workers, notifications, mobile, redesign, or Stage 3.77+ scope.
- This post-merge registry synchronization is documentation/governance only; it changes no runtime, OpenAPI, SQL migration, financial arithmetic, dependency or deployment behavior. Stage 3.76 becomes fully lifecycle/documentation closed when this approved closure reaches protected `develop` through the governance/closure path.

## OI-NEW P2 remediation canonical closure

- PR #183 — OI-NEW-02, OI-NEW-07; canonical merge `8ad6261ae61d057669f6e1a554e029ecc086394f`; required CI 10/10 PASS.
- PR #184 — OI-NEW-01; canonical merge `e72dd4d11c3143e2fc4db08c5d02cb6d3c80bcdf`; required CI 10/10 PASS.
- PR #185 — OI-NEW-05, OI-NEW-06; canonical merge `bc72f4be91a1bbdb17132271fac023fd212def36`; required CI 10/10 PASS.
- PR #186 — OI-NEW-03; canonical merge `a043c2143cfb7b08062f2375cd4e548e16ddcb90`; required CI 10/10 PASS.
- PR #187 — OI-NEW-04; canonical merge `eeb0554fe1a16d4668acdb5db37df8002e8434e0`; required CI 10/10 PASS.
- PR #188 — OI-NEW-08; canonical merge `d9f6c263dd3ff4d22fc978372014381880104489`; required CI 10/10 PASS.

P2 remediation closed 8/8 at
`develop@d9f6c263dd3ff4d22fc978372014381880104489`.

Documentation closure was subsequently canonicalized through PR #190 / squash merge `8bcb2d1bba3454de1262ddc4e045fddf299a9564` from exact head `b5bbfa9ea9cbb5eb9d758e573419e1d376e4fb21` after CI #548 / run `35334163751` with 10/10 required jobs successful.
