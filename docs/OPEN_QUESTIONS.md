...
| Next Review Date | Before any new architecture decision affecting provider/public activation, market valuation, privacy lifecycle, tax basis, imported SELL semantics, or Stage 3.76+ scope, or 2026-12-19, whichever comes first |

## Current questions

None. Stage 3.74 remains the last canonical implementation on protected `develop`. Stage 3.75 — Portfolio Cash Flow & Income Truth is a non-canonical Draft PR #152 implementation candidate and introduces no unresolved architecture question: it reuses the Stage 3.74 effective ledger, adds a bounded backend-owned aggregate/API and presentation-only UI, introduces no provider, migration or cache table, and keeps unrelated expenses out of `netInvestmentIncome` unless a reviewed row-level linkage exists. The pre-documentation-sync reviewed runtime head `83833ce0c791992e7f64d736dbf04e53c1de0298` passed CI #454 / run `34143875428` 10/10 SUCCESS; final candidate authority still requires green CI on the exact post-documentation-sync PR head and the normal human governance gates. Any future Stage 3.76+ provider/valuation, broader cost-basis/tax-lot semantics, imported SELL expansion or privacy-lifecycle implementation must enter this register if it changes frozen architecture.
