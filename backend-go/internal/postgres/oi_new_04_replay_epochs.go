...
func insertReplaySnapshotTx(ctx context.Context, tx *sql.Tx, portfolioID, snapshotDate string, now time.Time, state replayWorkingState) error {
	...
	result, err := tx.ExecContext(ctx, `
		...
		SELECT
			...
			COALESCE((
				...
				WHERE portfolio_id = $2 AND snapshot_date = $3::date AND methodology_version = $9
			), 1),
			$9, $10, $4
		FROM computed
		WHERE
	...
	`, uuid.NewString(), portfolioID, snapshotDate, now,
		cashValue.String(), positionValues.Stock.String(), positionValues.Bond.String(), invested.String(),
		stage371SnapshotMethodology, state.Financial.InputWatermark)
	if err != nil {
		return err
	}
...
func (s *Store) executeFinancialReplayPlanTx(
	...
	for index, snapshotDate := range affectedDates {
		...
		if err := insertReplaySnapshotTx(ctx, tx, portfolioID, snapshotDate, now, states[index]); err != nil {
			return err
		}
		countReplaySnapshotWrite(ctx)
	}
